...
def calculation_formula(t, omega, r, l):
    ...
    print(f"Formula of the equation of motion in LATEX form: {latex(x)}")
...
## This function generate the kinematic analysis base on the variable (engine properties) passed in
def kinematics():
    print("\nTo perform the kinematic analysis, please input your engine properties at the exact motor speed (The motor speed increases when accelerating and decreases when decelerate): \n")
    rV = float(input("Please enter the radius of the crank (in): "))
    lV = float(input("Please enter length of conrod (in): "))
    ...
    ## We plot the x, xdot, and xddot in 2 time period of rotation in second:
    ...
    plt.tight_layout()
    engineInfo = f"Engine Infomation: \nRadius of crank: {rV} in\nLength of conrol: {lV} in\nomega: {omegaRpm} rpm"
